...
                    produtosParaSalvar.push({ codigo: card.dataset.codigo, quantidade: quantidadeInput.value.replace('.', ','), validade: validadeInput.value || '' });
...
                const response = await fetch('/adicionar_item', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(dadosParaEnviar) });

                if (response.ok) { window.location.href = 'confirmacao.html'; }

                else { const erro = await response.json(); alert(`Erro ao salvar: ${erro.mensagem}`); }

            } catch (error) { console.error('Erro de comunicação com o servidor:', error); alert('Não foi possível conectar ao servidor para salvar os dados.'); }
